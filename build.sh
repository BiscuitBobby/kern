...
# Zip Name
ZIPNAME="Lemon_T"
LOG_FILE="LemonLog.log"

: > $LOG_FILE

# Specify compiler ( snapdragon_llvm, eva , azure , proton , arter , aosp & nexus )
COMPILER=nexus
...
function clone() {
    # Clone Toolchain
		...
		elif [ $COMPILER = "snapdragon_llvm" ]; then
		PATH="/opt/qcom/Qualcomm_Snapdragon_LLVM_ARM_Toolchain_OEM/10.0.9.0/toolchains/llvm/prebuilt/linux-x86_64/bin/clang"
		else
		# Assuming COMPILER is a path to clang
		post_msg " Using Custom Clang ToolChain "
		PATH="${COMPILER}:$PATH"
        fi
        # Clone AnyKernel3
